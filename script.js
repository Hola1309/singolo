...
send.addEventListener('click', (event) => {
	if (name.value != '' && email.value != '') {
		...
		if (a == '') {
			document.getElementById('subreturn').innerText = 'Без темы'
		} else {
			document.getElementById('subreturn').innerText = 'Тема: ' + a
		}
		a = document.getElementById('dis').value.toString();
		if (a == '') {
			document.getElementById('disreturn').innerText = 'Без описания'
		} else {
			document.getElementById('disreturn').innerText = 'Описание: ' + a
		}

		document.getElementById("get").className = "get";
	} else {
		alert('Ошибка, заполните поле name и email')
	}
});